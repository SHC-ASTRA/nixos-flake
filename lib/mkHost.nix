...
nixosModules ? [ ]
, # Additional NixOS modules to include (automatically includes hosts/name/default.nix)
...
let
  ...
  hostDefaultModule = if builtins.pathExists ../hosts/${name}/default.nix then
  ...
  allModules =
    lib.flatten (baseNixosModules ++ hostDefaultModule ++ nixosModules);
